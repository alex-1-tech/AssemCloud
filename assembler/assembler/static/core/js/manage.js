...
function buildTree($element) {
  const name = $element.children('.d-flex').find('input[name="assembly_name"]').val().trim();
  if (!name) return null;

  const parts = $element.children('.parts').children('.part').map(function () {
    const partName = $(this).find('input[name="part_name"]').val().trim();
    return partName ? { name: partName } : null;
  }).get().filter(Boolean);

  const subAssemblies = $element.children('.sub-assemblies').children('.assembly').map(function () {
...
function submitData() {
  ...
  const assemblies = $('#assemblies-root > .assembly').map(function () {
    return buildTree($(this));
  }).get().filter(Boolean);
