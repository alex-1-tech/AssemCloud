...
class TimeStampedModelWithUser(models.Model):
    ...
    updated_on = models.DateTimeField(
        auto_now=True, verbose_name=_("Обновлено")
    )  # Дата последнего изменения
    created_by = user_fk("%(class)s_created", _("Создано пользователем"))  # Кто создал
    updated_by = user_fk(
        "%(class)s_updated", _("Обновлено пользователем")
    )  # Кто обновил
    ...
    def clean(self):
        ...
        for field in self._meta.fields:
            ...
            normalize_method = getattr(
                self, f"normalize_{field_name}", None
            )  # Ищет метод normalize_<имя_поля>
            if callable(normalize_method):
                value = getattr(self, field_name)
                if value is not None:
                    setattr(
                        self, field_name, normalize_method(value)
